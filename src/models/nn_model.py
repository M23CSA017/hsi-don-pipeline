...
# #         val_loss = evaluate_model(model, val_loader)['Test_RMSE']
...
# #             y_true.extend(y_batch.cpu().numpy().flatten())  # Move to CPU for numpy
# #             y_pred.extend(y_pred_batch.cpu().numpy().flatten())  # Move to CPU for numpy
...
# #     # Train the model
# #     train_model(model, train_loader, val_loader, criterion, optimizer, model_path='./data/models/optuna_nn.pt',
...
# #     # Load the best model
# #     best_model = DONRegressor(X_train.shape[1], **study.best_trial.params)
# #     best_model.load_state_dict(torch.load('./data/models/optuna_nn.pt'))
...
#     best_model = load_model(best_model, './data/models/optuna_nn.pt')
