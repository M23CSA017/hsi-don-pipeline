...
def main():
    ...
    raw_data_path = config.get(
        "raw_data_path",
        "../data/raw/MLE-Assignment.csv")
    ...
    y_train_tensor = torch.tensor(
        y_train2.values.reshape(-1, 1), dtype=torch.float32)
    X_val_tensor = torch.tensor(X_test2.values, dtype=torch.float32)
    y_val_tensor = torch.tensor(
        y_test2.values.reshape(-1, 1), dtype=torch.float32)
    ...
    train_loader = prepare_dataloader(
        X_train2, y_train2, batch_size=best_params["batch_size"])
    val_loader = prepare_dataloader(
        X_test2, y_test2, batch_size=best_params["batch_size"])
    criterion = torch.nn.MSELoss()
    optimizer = torch.optim.AdamW(
        final_nn_model.parameters(),
    ...
    logger.info(
        f" Best NN model retrained and saved successfully to {NN_MODEL_PATH}")
    ...
    xgb_r2 = 1 - (np.sum((y_test2.values - y_pred_xgb) ** 2) /
                  np.sum((y_test2.values - np.mean(y_test2.values)) ** 2))

    logger.info(
        f" Best Neural Network Performance after Optuna: RMSE: {nn_metrics['Test_RMSE']:.4f}, R2: {nn_metrics['Test_R2']:.4f}")
    logger.info(
        f" Best XGBoost Model Performance: RMSE: {xgb_rmse:.4f}, R²: {xgb_r2:.4f}")
    logger.info(" Training pipeline completed successfully!")

    # --- Residual Plots ---
    residuals_nn_plot = perform_residual_analysis(
        y_test2.values,
        nn_metrics["Test_Predictions"],
        model_name="Neural Network")
    residuals_xgb_plot = perform_residual_analysis(
        y_test2.values, y_pred_xgb, model_name="XGBoost")
    logger.info(
        f"Residual plots saved to:\n NN: {residuals_nn_plot}\n XGB: {residuals_xgb_plot}")
