...
def perform_eda(df: pd.DataFrame, reflectance_cols: list) -> None:
    ...
    plt.plot(
        band_numbers,
        df[reflectance_cols].mean(),
        label="Average Reflectance")
    ...
    for band in selected_bands[:3]:
        ...
        plt.savefig(
            PLOT_DIR /
            f"hist_band_{float(band):.1f}.png",
            bbox_inches="tight")
        plt.show()
    ...
    boxplot_data.columns = [
        f"{float(col):.1f} nm" for col in boxplot_data.columns]
    ...
    for idx in sample_indices:
        smoothed = savgol_filter(
            df.loc[idx, reflectance_cols], window_length=11, polyorder=2)
    ...
    pairplot_data.columns = [
        f"{float(col):.1f} nm" for col in pairplot_data.columns]
...
def sensor_drift_check(df: pd.DataFrame, reflectance_cols: list) -> None:
    ...
    try:
        ...
        selected_bands = [reflectance_cols[0],
                          reflectance_cols[len(reflectance_cols) // 2],
                          reflectance_cols[-1]]
...
def preprocess_data(df: pd.DataFrame,
                    ...
                    nir_band: str = "100",
                    red_band: str = "50") -> Tuple[pd.DataFrame,
                                                   pd.DataFrame,
                                                   pd.DataFrame,
                                                   pd.DataFrame]:
    """
    ...
    logger.info(
        f"Split data: X_train {X_train.shape}, X_test {X_test.shape}, y_train {y_train.shape}, y_test {y_test.shape}")
    ...
    logger.info(
        f" PCA applied: 70 components retained, explaining {explained_var:.2%} variance.")
    ...
    logger.info(
        f"Final shapes before return: X_train {X_train_final.shape}, "
